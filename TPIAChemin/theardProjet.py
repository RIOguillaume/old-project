...
def valueR2():
    return lancerR2
def thread_R1_function(name,cv):
    global lancerR1
    global lancerR2
    global lancerConv
    global cpt
    numKaplaCourrent = 0
    while(cpt < len(tabTrier)):
        numKaplaCourrent = cpt
        """
        pos = posStockage(cpt)
        ...
        posCubeStockageR = 0
        """
        (posCubeStockageX,posCubeStockageY,posCubeStockageZ) = posStockage(numKaplaCourrent)
        ...
        #on attend que le bras 2 est fini pour lacher le kapla sur le convoyeur
        if (lancerR2):
            while lancerR2:
                print("1 bloqué car 2 est en route")
                time.sleep(0.01)
        ...
        vitesse = 100
        vitesse_ralentie = 30
        api1.conveyor_belt(vitesse,-1, 0)
        offset_rot_angle = 0
        if numeroPosFinConvoyeur(numKaplaCourrent) == 0:
            ...
            while not cam.ligneVirtuelleTrigerred():
                time.sleep(0.05)
            api1.conveyor_belt(vitesse_ralentie,-1, 0)
        ...
        if numeroPosFinConvoyeur(numKaplaCourrent) != 0:
            boiteAction(4)
            time.sleep(5)
            if numeroPosFinConvoyeur(numKaplaCourrent) == 2:
                boiteAction(3)
            else:
                boiteAction(1)
            time.sleep(5)
...
def thread_R2_function(name,cv):
    global lancerR1
    global lancerR2
    global lancerConv
    global cpt
    ...
    while(cpt < len(tabTrier)):
        ...
        numKaplaCourrent = cpt
        (posConvoyeurFinX,posConvoyeurFinY,posConvoyeurFinZ) = posConvoyeur(numKaplaCourrent)
        """
        ...
        #on lance R1 sur le prochain kapla
        cpt +=1
        lancerR1 = True
        with cv:
            cv.notify_all()

        (posConstructionFinX,posConstructionFinY,posConstructionFinZ,rotContructionFin)=positionFinalConstruction(numKaplaCourrent, offset_rot_angle)
        """
...
R1 = threading.Thread(target=thread_R1_function, args=(1,condition))
R2 = threading.Thread(target=thread_R2_function, args=(2,condition))
R1.start()
R2.start()
while(True):
    continue
